...
PrioVis = function(_parentElement, _data, _metaData, _geoJSON){
...
PrioVis.prototype.wrangleData = function(){
    ...
    vis.votesPerPriority = d3.range(0,15).map(function() {
        return 0;
    });
    ...
    vis.filteredData.forEach(function(day){
        d3.range(0,15).forEach(function(i){
            vis.votesPerPriority[i] += day["priorities"][i];
        });
    });

    vis.displayData = vis.votesPerPriority;
...
PrioVis.prototype.onSelectionChange = function(selectionStart, selectionEnd){
    ...
    vis.filteredData = vis.data.filter(function(d){
        return d.time < selectionEnd && d.time > selectionStart;
    });
    vis.wrangleData();
}
